    ...
    try:
        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                response = listener.recognize_google(voice).lower()
                print(f'User response: {response}')
                if len(response.strip()) == 0:  # Handle empty response
                    raise sr.UnknownValueError()
                handle_another_emotion_(response)
                return True  # If response is valid, stop the loop
            except sr.WaitTimeoutError:
                # Trigger emergency action if no response is received within 5 seconds
                print("Emergency! No response received. Making a call now......")
                speak_and_print("Emergency! No response received. Making a call now......")
                time.sleep(10)
...
def run_assistant():
    ...
    initial_emotion_check_done = False  # Flag to prevent repeated "Are you okay?"

    try:
        while True:
            # Run the emotion handling first only once to ask "Are you okay?"
            while not initial_emotion_check_done:
                ask_are_you_okay()  # Call the function to ask the question
            ...
            try:
                ...
                with sr.Microphone() as source:
                    ...
                    elif 'relax' in command or 'relaxing' in command or 'relaxed' in command:
                        print("Playing music......")
                        play_music()
                        print('\n\n')
                        time.sleep(1)
